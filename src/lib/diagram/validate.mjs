...
export function validateDiagramScene(scene, options = {}) {
  ...
  const errors = [];
  const warnings = [];
  if (!isObject(scene)) return { errors: [`${source}: 図データはオブジェクトである必要があります。`], warnings };
  ...
  for (const [index, element] of scene.elements.entries()) {
    ...
    if (typeof element?.id === "string" && element.id.length > 0) {
      if (ids.has(element.id)) errors.push(`${location}: id「${element.id}」が図内で重複しています。`);
...
function validateElement(element, location, errors) {
  ...
  if (element.kind === "symbol") {
    ...
    if (!symbols || !symbols.has(element.symbol)) {
      errors.push(`${location}: 未対応の専用記号 ${String(element.domain)}:${String(element.symbol)} です。`);
    }
    validatePoint(element.at, `${location}.at`, errors);
    if (element.width !== undefined && !positive(element.width)) errors.push(`${location}.width は正の有限数にしてください。`);
    if (element.height !== undefined && !positive(element.height)) errors.push(`${location}.height は正の有限数にしてください。`);
    if (element.rotation !== undefined && !finite(element.rotation)) errors.push(`${location}.rotation は有限数にしてください。`);
  ...
  if (element.refs !== undefined && (!Array.isArray(element.refs) || element.refs.some((ref) => typeof ref !== "string"))) {
...
function validateCoreElement(element, location, errors) {
  switch (element.kind) {
    case "point":
      ...
      if (element.radius !== undefined && !positive(element.radius)) errors.push(`${location}.radius は正の有限数にしてください。`);
      break;
    case "segment":
    case "line":
    case "arrow":
    ...
    case "ellipse":
      validatePoint(element.center, `${location}.center`, errors);
      if (!positive(element.radiusX)) errors.push(`${location}.radiusX は正の有限数にしてください。`);
      if (!positive(element.radiusY)) errors.push(`${location}.radiusY は正の有限数にしてください。`);
      break;
    case "polygon":
      if (!Array.isArray(element.points) || element.points.length < 3) errors.push(`${location}.points は3点以上必要です。`);
      else element.points.forEach((value, index) => validatePoint(value, `${location}.points[${index}]`, errors));
      break;
    case "arc":
      validatePoint(element.center, `${location}.center`, errors);
      if (!positive(element.radius)) errors.push(`${location}.radius は正の有限数にしてください。`);
      if (!finite(element.startAngle) || !finite(element.endAngle)) errors.push(`${location}: arc の角度は有限数にしてください。`);
      break;
    case "label":
      validatePoint(element.at, `${location}.at`, errors);
      if (typeof element.text !== "string") errors.push(`${location}.text は文字列にしてください。`);
      break;
    case "axes":
      validatePoint(element.origin, `${location}.origin`, errors);
      for (const key of ["xMin", "xMax", "yMin", "yMax"]) if (!finite(element[key])) errors.push(`${location}.${key} は有限数にしてください。`);
      if (element.gridStep !== undefined && !positive(element.gridStep)) errors.push(`${location}.gridStep は正の有限数にしてください。`);
      break;
    case "functionPlot":
      if (!Array.isArray(element.samples) || element.samples.length < 2) errors.push(`${location}.samples は2点以上必要です。`);
      else element.samples.forEach((value, index) => validatePoint(value, `${location}.samples[${index}]`, errors));
      if (element.expression !== undefined && typeof element.expression !== "string") errors.push(`${location}.expression は文字列にしてください。`);
      break;
  }
}
...
function validateConstraint(constraint, location, ids, errors, warnings) {
  ...
  for (const ref of refs) if (!ids.has(ref)) errors.push(`${location}: 参照先 id「${ref}」がありません。`);
  ...
  if (constraint.kind === "circuitConnection") {
    for (const ref of refs) {
      ...
      if (element.kind === "symbol" && element.domain !== "circuit") {
        errors.push(`${location}: circuitConnection は回路記号または基本図形だけを参照してください。`);
      }
    }
  }
  ...
  if (constraint.kind === "order") {
    ...
    if (anchors.length === refs.length) {
      ...
      if (!values.every((value, index) => index === 0 || values[index - 1] <= value)) warnings.push(`${location}: order 制約を現在の基準座標が満たしていません。近似配置の対象です。`);
    }
    return;
  }
  ...
  if (constraint.kind === "inside") {
    ...
    if (inner && outer && !contains(outer, inner)) warnings.push(`${location}: inside 制約を現在の基準座標が満たしていません。近似配置の対象です。`);
    return;
  }
  ...
  if (constraint.kind === "parallel" || constraint.kind === "perpendicular") {
    ...
    const difference = constraint.kind === "parallel" ? Math.min(angle, Math.abs(180 - angle)) : Math.abs(angle - target);
    if (difference > (constraint.tolerance ?? 1)) warnings.push(`${location}: ${constraint.kind} 制約を現在の基準座標が満たしていません。`);
    return;
  }
  ...
  if (constraint.kind === "angle") {
    ...
    const difference = Math.min(Math.abs(actual - normalizeDegrees(constraint.degrees)), 360 - Math.abs(actual - normalizeDegrees(constraint.degrees)));
    if (difference > (constraint.tolerance ?? 1)) warnings.push(`${location}: angle 制約を現在の基準座標が満たしていません。`);
    return;
  }
  ...
  if (constraint.kind === "distance") {
    ...
    if (Math.abs(actual - constraint.value) > (constraint.tolerance ?? 1)) warnings.push(`${location}: distance 制約を現在の基準座標が満たしていません。`);
    return;
  }
  ...
  if (constraint.kind === "ratio") {
    ...
    if (Math.abs(actual - constraint.value) > (constraint.tolerance ?? 0.02)) warnings.push(`${location}: ratio 制約を現在の基準座標が満たしていません。`);
  }
}
...
function anchorOf(element) {
  ...
  if (["segment", "line", "arrow"].includes(element.kind)) return { x: (element.from.x + element.to.x) / 2, y: (element.from.y + element.to.y) / 2 };
...
function contains(outer, inner) {
  if (outer.kind === "circle") return Math.hypot(inner.x - outer.center.x, inner.y - outer.center.y) <= outer.radius;
  ...
  if (outer.kind === "polygon") {
    ...
    return inner.x >= Math.min(...xs) && inner.x <= Math.max(...xs) && inner.y >= Math.min(...ys) && inner.y <= Math.max(...ys);
  }
  return true;
}
...
function centroid(points) {
  ...
  return points.reduce((sum, value) => ({ x: sum.x + value.x / points.length, y: sum.y + value.y / points.length }), { x: 0, y: 0 });
}
...
function validatePoint(value, location, errors) {
  if (!isObject(value) || !finite(value.x) || !finite(value.y)) errors.push(`${location}: x, y は有限数にしてください。`);
}
