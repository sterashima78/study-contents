...
export const broaderFunctionRelationsLesson: MathLesson = {
  ...
  description:
    "比例・一次関数・y=ax²以外でも、一方を決めると他方がただ一つ決まる関係を関数として捉えます。",
  goals: ["関数の意味を式の形に限定せず説明できる。", "段階的な料金などを表やグラフで考察できる。"],
  concepts: [
    {
      title: "関数は式の種類ではなく対応の考え",
      body: [
        "料金表のように一つの簡単な式で表しにくい関係でも、入力を一つ決めれば出力がただ一つ決まるなら関数です。",
        "表やグラフを使えば、式が複雑でも変化や対応の特徴を調べられます。",
      ],
  ...
  practice: {
    ...
    steps: [
      {
        prompt: "時刻を一つ決めると、その時刻の気温が一つ決まる関係は関数とみなせますか。",
        answers: ["はい", "関数", "関数です"],
        placeholder: "はい/いいえ",
      },
      {
