...
function constructionSymmetryScene(): DiagramScene {
  ...
  return {
    width: 520,
    height: 340,
    ariaLabel: "線分ABの両端を中心とする同じ半径の二つの円がPとQで交わり、直線PQがABの対称軸になる図。",
    ...
    elements: [
      ...
      dot(a, BLUE), dot(b, GREEN), dot(p), dot(q),
      label(point(128, 200), "A", BLUE), label(point(392, 200), "B", GREEN),
      label(point(278, 82), "P", ORANGE, "start"), label(point(278, 286), "Q", ORANGE, "start"),
    ],
  };
}
...
function angleBisectorScene(): DiagramScene {
  ...
  return {
    width: 520,
    height: 340,
    ariaLabel: "角XOYの内部に半直線OPがあり、OPが角を二等分する作図の図。円弧の交点を使って対称軸を作る。",
    responsive: { minWidth: 440, allowHorizontalScroll: true },
    elements: [
      ...
      { kind: "arc", center: point(172, 208), radius: 130, startAngle: -35, endAngle: 15, color: GREEN },
      { kind: "arc", center: point(200, 275), radius: 130, startAngle: -75, endAngle: -25, color: GREEN },
      dot(o, BASE), dot(p),
      label(point(90, 296), "O"), label(point(446, 284), "X"), label(point(323, 57), "Y"), label(point(381, 158), "P", ORANGE, "start"),
    ],
  };
}
...
function perpendicularBisectorScene(): DiagramScene {
  ...
  return {
    width: 520,
    height: 340,
    ariaLabel: "線分ABの両端を中心とする同じ半径の円の交点P、Qを結び、ABの垂直二等分線を作図する図。",
    responsive: { minWidth: 440, allowHorizontalScroll: true },
    elements: [
      { kind: "segment", from: a, to: b, color: BASE },
      { kind: "circle", center: a, radius: 150, color: BLUE },
      ...
      dot(a, BLUE), dot(b, GREEN), dot(p), dot(q),
      label(point(128, 201), "A", BLUE), label(point(392, 201), "B", GREEN),
      label(point(279, 90), "P", ORANGE, "start"), label(point(279, 274), "Q", ORANGE, "start"),
...
function perpendicularScene(): DiagramScene {
  ...
  return {
    ...
    elements: [
      ...
      dot(a, BLUE), dot(b, GREEN), dot(p), dot(q),
      label(point(475, 211), "l", BASE, "start"), label(point(276, 73), "P", ORANGE, "start"),
      label(point(276, 330), "Q", ORANGE, "start"), label(point(279, 190), "90°", PURPLE, "start"),
    ],
  };
}
...
function translationScene(): DiagramScene {
  ...
  return {
    width: 520,
    height: 340,
    ariaLabel: "青い三角形ABCを右上へ平行移動し、橙色の三角形A'B'C'に重ねる図。対応点を結ぶ矢印は平行で同じ長さ。",
    responsive: { minWidth: 440, allowHorizontalScroll: true },
    elements: [
      { kind: "polygon", points: original, color: BLUE },
      { kind: "polygon", points: moved, color: ORANGE },
      ...original.map((from, index): DiagramElement => ({ kind: "arrow", from, to: moved[index], color: GREEN })),
      label(point(83, 276), "A", BLUE), label(point(190, 276), "B", BLUE), label(point(135, 145), "C", BLUE),
      label(point(278, 196), "A'", ORANGE), label(point(386, 196), "B'", ORANGE), label(point(330, 65), "C'", ORANGE),
    ],
  };
}
...
function reflectionScene(): DiagramScene {
  ...
  return {
    ...
    elements: [
      ...
      { kind: "polygon", points: original, color: BLUE },
      { kind: "polygon", points: moved, color: ORANGE },
      ...original.map((from, index): DiagramElement => ({ kind: "segment", from, to: moved[index], color: GREEN })),
      label(point(273, 57), "対称軸 l", PURPLE, "start"),
      label(point(100, 285), "A", BLUE), label(point(198, 267), "B", BLUE), label(point(150, 110), "C", BLUE),
      label(point(420, 285), "A'", ORANGE), label(point(322, 267), "B'", ORANGE), label(point(370, 110), "C'", ORANGE),
    ],
  };
}
...
function rotationScene(): DiagramScene {
  ...
  return {
    ...
    elements: [
      ...
      dot(o, PURPLE),
      label(point(247, 241), "O", PURPLE), label(point(302, 185), "90°", PURPLE),
      label(point(356, 226), "A", BLUE, "start"), label(point(356, 136), "B", BLUE, "start"),
      label(point(260, 125), "A'", ORANGE), label(point(164, 136), "B'", ORANGE, "end"),
    ],
  };
}
...
function tangentScene(): DiagramScene {
  ...
  return {
    ...
    elements: [
      ...
      dot(o, GREEN), dot(t, ORANGE),
      label(point(232, 201), "O", GREEN), label(point(374, 178), "T", ORANGE, "start"),
      label(point(372, 60), "接線 l", ORANGE, "start"), label(point(320, 165), "半径", GREEN),
...
const diagrams: Record<string, MathLessonDiagrams> = {
  "construction-symmetry": { rule: constructionSymmetryScene(), example: constructionSymmetryScene() },
