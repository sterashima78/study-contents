...
const linearExpression = (a: number, b: number) => `${firstTerm(a, "x")}${signText(b)}`;
...
const expansionGenerator: Generator = () => {
  ...
  return spec(
    `${binomial(a, b)}(x ${c >= 0 ? "+" : "−"} ${Math.abs(c)}) を展開してください。`,
    [answer],
    ["expansion"],
    "分配法則で4つの積を作り、最後に同類項をまとめます。",
  );
};
...
const sumDifferenceGenerator: Generator = () => {
  ...
  return spec(
    `${binomial(a, b)}${binomial(a, -b)} を展開してください。`,
    [answer],
    ["multiplication-formula-sum-difference"],
    "(a + b)(a − b) = a² − b² を使います。",
  );
...
const commonFactorGenerator: Generator = () => {
  ...
  return spec(
    `${promptExpression} を共通因数で因数分解してください。`,
    [answer],
    ["common-factor"],
    "数の最大公約数と、すべての項に共通する x を探します。",
  );
};
...
const quadraticFactorizationGenerator: Generator = () => {
  ...
  return spec(
    `${promptExpression} を因数分解してください。`,
    [`${first}${second}`, `${second}${first}`],
    ["quadratic-factorization"],
    "和が x の係数、積が定数項になる2数を探します。",
  );
};
...
const radicalsGenerator: Generator = () => {
  ...
  return spec(
    ...
    [`${k}√${n}`],
    ["radicals"],
    `${radicand} の中に ${k * k} という平方数が含まれています。`,
  );
};
...
const absoluteValueGenerator: Generator = () => {
  ...
  return spec(
    `|−${a}| ${operator} |${b}| を計算してください。`,
    [String(answer)],
    ["absolute-value"],
    "絶対値を先に1つずつ外してから計算します。",
  );
};
...
const inequalityRulesGenerator: Generator = () => {
  ...
  return spec(
    `${left} < ${right} の両辺に ${multiplier} を掛けた結果を、不等号まで含めて書いてください。`,
    [`${left * multiplier}>${right * multiplier}`],
    ["inequality-rules"],
    "負の数を掛けるので、不等号の向きを反転します。",
  );
};
...
const solvingGenerator: Generator = () => {
  ...
  return spec(
    `${linearExpression(a, b)} ${operator} ${right} を解いてください。`,
    [`x${solvedOperator}${solution}`],
    ["solving", ...(a < 0 ? ["inequality-rules"] : [])],
    a < 0
      ? "最後に負の数で割るため、不等号を反転します。"
      : "x の項と定数項を分けてから係数で割ります。",
  );
};
...
const wordProblemsGenerator: Generator = () => {
  ...
  return spec(
    `1個${price}円の商品を買い、固定費${fixed}円を含めて${budget}円以下にします。最大何個買えますか。`,
    [String(maxCount), `${maxCount}個`],
    ["word-problems", "solving"],
    `個数を x として ${price}x + ${fixed} ≤ ${budget} と置きます。`,
  );
};
...
const setOperationsGenerator: Generator = () => {
  ...
  const result =
    operation === "union" ? orderedSet([...a, ...b]) : a.filter((value) => b.includes(value));
  return spec(
    `A={${a.join(",")}}, B={${b.join(",")}} の ${symbol} を小さい順に書いてください。`,
    setAnswer(result),
    ["set-operations"],
    operation === "union"
      ? "和集合は少なくとも一方に含まれる要素です。"
      : "共通部分は両方に含まれる要素です。",
  );
};
...
const counterexampleGenerator: Generator = () => {
  ...
  return spec(
    `命題「x² ≥ ${k * k} ならば x ≥ ${k}」の反例となる整数を1つ書いてください。`,
    [`-${k}`, `−${k}`, `-${k + 1}`, `−${k + 1}`, `-${k + 2}`, `−${k + 2}`],
    ["propositions-counterexamples"],
    `負の整数で、絶対値が ${k} 以上のものを試します。`,
  );
};
...
const contrapositiveGenerator: Generator = () => {
  ...
  return spec(
    `命題「x > ${upper} ならば x > ${lower}」の対偶を書いてください。`,
    [`x≤${lower}⇒x≤${upper}`, `x≤${lower}ならばx≤${upper}`],
    ["contrapositive"],
    "結論を否定し、条件を否定して、順番を逆にします。",
  );
};
...
const expansionApplied: Generator[] = [
  () => {
    ...
    return spec(
      `(${a}x + ${b})² − (${a}x − ${b})² を展開して簡単にしてください。`,
      [`${4 * a * b}x`],
      ["multiplication-formulas", "multiplication-formula-difference-square"],
      "2つの二乗公式をそれぞれ展開すると、二次項と定数項が消えます。",
    );
  },
  () => {
    ...
    return spec(
      `${expanded} をできるだけ因数分解してください。`,
      [`${g}(x+${m})(x+${n})`, `${g}(x+${n})(x+${m})`],
      ["common-factor", "quadratic-factorization"],
      `まず共通因数 ${g} をくくり、その後 x² + px + q の形を因数分解します。`,
    );
  },
  () => {
    ...
    return spec(
      `(${a}x + ${b})(${a}x − ${b}) + (${c}x + ${d})² を展開して簡単にしてください。`,
      [answer],
      ["multiplication-formula-sum-difference", "multiplication-formulas"],
      "和と差の積と、和の二乗を別々に展開してから足します。",
    );
  },
];
...
const realApplied: Generator[] = [
  ...
  () => {
    ...
    return spec(
      `${p}/√${q} + √${q} を有理化した形でまとめてください。`,
      [`${p + q}√${q}/${q}`],
      ["rationalization", "radicals"],
      `√${q} を ${q}√${q}/${q} とみると通分できます。`,
    );
  },
  () => {
...
const inequalityApplied: Generator[] = [
  ...
  () => {
    ...
    return spec(
      `${a}x + ${b} ≤ ${c}x + ${d} を解いてください。`,
      [`x${operator}${boundary}`],
      ["solving", "inequality-rules"],
      "x の項を一方へ集め、負の係数で割る場合は不等号を反転します。",
    );
  },
];
...
const setsApplied: Generator[] = [
  () => {
    ...
    const c = [2, 4, 5, 8];
    return spec(
      `A={${a}}, B={${b}}, C={${c}} の (A ∪ B) ∩ C を求めてください。`,
      ["{2,4}", "2,4"],
      ["set-operations"],
      "先に A ∪ B を作り、その結果と C の共通部分を取ります。",
    );
  },
  () => {
    const k = randomInt(2, 5);
    return spec(
      `命題「x² = ${k * k} ならば x = ${k}」は真か偽か。`,
      ["偽"],
      ["propositions-counterexamples"],
      `x = −${k} を反例として確認します。`,
    );
  ...
  () => {
    ...
    return spec(
      `p: x > ${upper}, q: x > ${lower} とします。p ⇒ q の対偶を書いてください。`,
      [`x≤${lower}⇒x≤${upper}`, `x≤${lower}ならばx≤${upper}`],
      ["necessary-sufficient", "contrapositive"],
      "p ⇒ q が成り立つことを確認し、¬q ⇒ ¬p を作ります。",
    );
  },
];
...
const unitChallengeGenerators: Record<string, Generator> = {
  "expansion-factorization": () => {
    ...
    return spec(
      `(${a}x + ${b})(${c}x + ${d}) − (${e}x + ${f})² を展開して簡単にしてください。`,
      [answer],
      ["expansion", "multiplication-formulas"],
      "最初の積は分配法則、後半は和の二乗の公式で展開してから引きます。",
    );
  },
  "real-numbers": () =>
    spec(
      "2/√3 + √12 − |−1| を有理化した形で簡単にしてください。",
      ["8√3/3-1", "−1+8√3/3"],
      ["rationalization", "radicals", "absolute-value"],
      "2/√3 を有理化し、√12 を 2√3 に直してから同類の根号をまとめます。",
    ),
  "linear-inequalities": () =>
    spec(
      "料金Aは 800 + 120x 円、料金Bは 2000 + 60x 円です。料金Aが料金B以上になる x の範囲を求めてください。",
      ["x≥20"],
      ["word-problems", "solving", "inequality-rules"],
      "800 + 120x ≥ 2000 + 60x と置いて解きます。",
    ),
  "sets-propositions": () =>
    spec(
      "p: x > 4, q: x > 1 とします。p は q の十分条件であることを踏まえ、p ⇒ q の対偶を書いてください。",
      ["x≤1⇒x≤4", "x≤1ならばx≤4"],
      ["necessary-sufficient", "contrapositive"],
      "p ⇒ q の対偶は ¬q ⇒ ¬p です。",
    ),
...
export const generateUnitExercises = (unitKey: string) => {
  ...
  const applied = appliedGenerators
    .slice(0, 3)
    .map((generator, index) => finalize(generator(), "applied", index));
