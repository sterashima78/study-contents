...
説明用の `DiagramView` は75%から250%のズームを提供する。100%では `responsive.minWidth` を読みやすさの基準として使い、狭い画面では必要に応じて横スクロールする。利用者が縮小を選んだ場合は有効な最小幅も同じ倍率で縮小するため、モバイルでも図全体を画面内へ収めやすくなる。
