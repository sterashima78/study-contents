# ADR 0023: グローバルCSSを撤去し基本スタイルをレイアウトコンポーネントへ集約する
...
この構成でも影響範囲は大幅に限定されたが、ページの基礎スタイルだけがコンポーネント外のファイルに残るため、画面の表示責務を追う際に `StudyPage.astro` と `global.css` の2箇所を確認する必要がある。また、将来 `global.css` に別の共通セレクタを追加してしまう余地も残る。
...
`src/styles/global.css` を削除し、新しいグローバルCSSファイルを作らない。

ページ全体の基本表示に必要なスタイルは `src/components/ui/StudyPage.astro` が所有する。
...
`body` や `html` を操作するために別CSSファイルを作らず、どのレイアウトがグローバルな文書スタイルを所有するかをソース上で明確にする。
...
### 7. CIで `global.css` の不在を保証する

`scripts/verify-component-styles.mjs` は次を検証する。

- `src/styles/global.css` が存在しないこと
- Astroファイルが `global.css` を参照していないこと
...
一方、`html` と `body` への指定だけは文書ルートへ作用する必要があるため、Astroの `:global(...)` を `StudyPage.astro` 内で使用する。これはグローバルCSSファイルを再導入することとは区別し、文書レイアウトの責務として管理する。
